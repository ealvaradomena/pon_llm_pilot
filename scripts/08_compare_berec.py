"""Compare the three BEREC classification conditions.

This script performs local analysis only. It makes no API calls.

Conditions
----------
1. Preserved historical Dec. 23, 2024 BEREC output.
2. Best-evidenced Dec. 23 reconstruction with GPT-4 Turbo.
3. Dec. 24 SBS protocol replication with the newer model.
...
def parse_binary_csv(path: Path) -> list[tuple[str, str]]:
    ...
    for line in cleaned[1:]:
        ...
        # Newer-model output may leave questions unquoted even when
