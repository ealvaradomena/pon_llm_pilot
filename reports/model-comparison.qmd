...
paths <- list(
  evaluation = "derived/sbs_model_evaluation_long.csv",
...
if (comparison_ready) {
  full_evaluation <- readr::read_csv(paths$evaluation, show_col_types = FALSE)
  full_flips <- readr::read_csv(paths$flips, show_col_types = FALSE)
  ...
  comparison_ready <- setequal(full_counts$model_id, expected_model_ids) &&
    all(full_counts$n == 75L) &&
    all(comparison_models$model_id %in% full_flips$comparison_model_id) &&
...
if (comparison_ready) {
  ...
  stability <- flips |>
    ...
    dplyr::summarise(
      decisions = dplyr::n(),
      same = sum(!.data$changed),
      changed = sum(.data$changed),
      agreement_rate = mean(!.data$changed),
      .groups = "drop"
    )
  ...
  published_decisions <- overall |> dplyr::pull(.data$n) |> unique()
  ...
  by_pon_display <- by_pon |>
    ...
    dplyr::mutate(
      change_vs_legacy_pp = dplyr::if_else(
        .data$model_id == legacy_model_id, NA_real_,
...
This comparison holds the **protocol, five PONs, published question subset, and human reference fixed** while varying the model arm. The historical GPT-4 Turbo arm is preserved rather than rerun; the two contemporary arms were executed independently under the reconstructed Dec. 24, 2024 SBS protocol.
...
if (comparison_ready) {
  cat(sprintf(
    "Across the published PON-question classifications, the frozen GPT-4 Turbo SBS responses agree with the confirmed human reference in **%s** of cases.\n\n",
  ...
  for (i in seq_len(nrow(contemporary_summary))) {
    ...
    cat(sprintf(
      paste0(
        ...
        "It changes **%s of %s classifications (%s)**: **%s toward** the human reference and **%s away**.\n\n"
      ),
      row$model,
      ...
      flips_row$changed,
      published_decisions,
      scales::percent(flips_row$changed / published_decisions, accuracy = 0.1),
      flips_row$toward,
      flips_row$away
    ))
  }
...
The results are descriptive of this fixed five-PON published comparison. They do not establish that one model is generally superior across other tasks, prompts, or domains.
...
```{r}
#| label: tbl-majority
#| tbl-cap: "LLM agreement with the three-human majority across the published question subset"
if (comparison_ready) {
  overall |>
    dplyr::transmute(
      Model = .data$model,
...
The comparison script validates the complete computational artifacts before writing results. This page then applies the centrally configured publication exclusion before calculating or displaying website statistics. Each contemporary model is compared directly with the frozen GPT-4 Turbo outputs. Historical outputs remain untouched regardless of parsing or scoring results.
